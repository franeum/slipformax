...
void unpackOSC_free(t_unpackOSC *x);
void unpackOSC_list(t_unpackOSC *x, t_symbol *s, int argc, t_atom *argv);
int unpackOSC_path(t_atom *data_at, char *path);
void unpackOSC_Smessage(t_atom *data_at, int *data_atc, void *v, int n);
void unpackOSC_PrintTypeTaggedArgs(t_atom *data_at, int *data_atc, void *v, int n);
void unpackOSC_PrintHeuristicallyTypeGuessedArgs(t_atom *data_at, int *data_atc, void *v, int n, int skipComma);
char *unpackOSC_DataAfterAlignedString(char *string, char *boundary);
int unpackOSC_IsNiceString(char *string, char *boundary);
double unpackOSC_DeltaTime(OSCTimeTag tt);

void *unpackOSC_new(void)
{
    t_unpackOSC *x;
    x = (t_unpackOSC *)object_alloc(unpackOSC_class);
    
    x->x_delay_out = floatout(x);
    x->x_data_out = listout(x);
    x->x_bundle_flag = 0;
...
void ext_main(void *r)
{
    ...
    c = class_new("unpackosc",
        (method)unpackOSC_new, (method)unpackOSC_free,
        sizeof(t_unpackOSC), 0, 0);
    
    class_addmethod(c, (method)unpackOSC_list, "list", A_GIMME, 0);
...
#ifdef DEBUG
    post(">>> unpackOSC_list: %d bytes, abort=%d, reentry_count %d recursion_level %d\n",
...
/* , so change to this: */
            if ((j == argv[i].a_w.w_long) && (j >= -128) && (j <= 255))
            {
                raw[i] = (char)j;
            }
            else
        ...
        {
            post("unpackOSC: Data not integer, dropping packet");
            goto unpackOSC_list_out;
        }
    }
    raw_c = argc;
...
/* convert the timetag into a millisecond delay from now */
    ...
    if (data_atc >= 1)
        outlet_anything(x->x_data_out, atom_getsym(data_at), data_atc-1, data_at+1);
...
int unpackOSC_path(t_atom *data_at, char *path)
{
    ...
    for (i = 1; i < MAX_MESG; ++i)
    {
        ...
        { /* the end of the path: turn path into a symbol */
            atom_setsym(data_at, gensym(path));
            return 1;
        }
    }
        ...
        switch (*thisType)
        {
            case 'b': /* blob: an int32 size count followed by that many 8-bit bytes */
            {
                ...
                for (i = 0; i < blob_bytes; ++i, ++p, ++myargc)
                    atom_setfloat(mya+myargc,(*(unsigned char *)p));
                while (i%4)
                {
                    ++i;
                    ++p;
            ...
            case 'm': /* MIDI message is the next four bytes*/
            {
                ...
                for (i = 0; i < 4; ++i, ++p, ++myargc)
                    atom_setlong(mya+myargc, (*(unsigned char *)p));
                break;
            }
            case 'i': case 'r': case 'c':
                ...
                printf("integer: %d\n", ntohl(*((int *) p)));
#endif
                atom_setlong(mya+myargc,(signed)ntohl(*((int *) p)));
                myargc++;
                p += 4;
                break;
            case 'f':
            {
                ...
                printf("float: %f\n", thisif.f);
#endif
                atom_setfloat(mya+myargc, thisif.f);
                myargc++;
                p += 4;
                break;
            }
            ...
            case 's': case 'S':
                ...
                else
                {
                    ...
                    printf("string: \"%s\"\n", p);
#endif
                    atom_setsym(mya+myargc,gensym(p));
                    myargc++;
            ...
            case 'I':
                ...
                printf("[Infinitum]\n");
#endif
                atom_setsym(mya+myargc,gensym("INF"));
                myargc++;
                break;
            default:
#ifdef DEBUG
...
void unpackOSC_PrintHeuristicallyTypeGuessedArgs(t_atom *data_at, int *data_atc, void *v, int n, int skipComma)
{
    ...
    for (i = 0; i < n/4; )
    {
        ...
        if (thisif.i >= -1000 && thisif.i <= 1000000)
        {
            ...
            printf("%d ", thisif.i);
#endif
            atom_setfloat(mya+myargc,(double)(thisif.i));
            myargc++;
            i++;
        }
...
#endif
            atom_setsym(mya+myargc,gensym(string));
            myargc++;
...
double unpackOSC_DeltaTime(OSCTimeTag tt)
{
    double onemillion = 1000000.0f;
#ifdef _WIN32
    double onethousand = 1000.0f;
#endif /* ifdef _WIN32 */
